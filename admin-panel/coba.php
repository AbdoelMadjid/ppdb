...
<body>
	...
	//koneksi ke database
	$conn = new mysqli("localhost", "root", "", "dbppdb");
	...
	if($_POST['submit']){
		...
		$salt ='vieyama';
		$password_lama	= md5($salt . $password_lama);
		$cek 			= $conn->query("SELECT password FROM pengguna WHERE password='$password_lama'");
		...
		if($cek->num_rows){
			...
			if(strlen($password_baru) >= 5){
				...
				if($password_baru == $konfirmasi_password){
					...
					$id_user 		= $_SESSION['nisn']; //ini dari session saat login

					$update 		= $conn->query("UPDATE pengguna SET password='$password_baru' WHERE nisn='$id_user'");
					if($update){
